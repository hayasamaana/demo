...
import geometry.petri.geometry.diagram.part.GeometryDiagramEditor;
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof GeometryNavigatorItem) {
					selectedElement = ((GeometryNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (GeometryEditPart.MODEL_ID.equals(GeometryVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		*/
		public void run() {
			...
			try {
				...
				GeometryDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
